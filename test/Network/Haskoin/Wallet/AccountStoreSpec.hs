...
spec = prepareContext $ \ctx -> do
  ...
  describe "AccountMap" $ do
    ...
    it "Inserting duplicate accounts fails" $ do
      let a1 =
            exec s1 $
              insertAccountStore "acc1" (emptyAccountStore btc pub2)
          a2 =
            exec s1 $
              insertAccountStore "acc2" (emptyAccountStore btc pub1)
    ...
    it "Can add a second account" $
      assertEqual
        "map"
        ( AccountMap
            (Map.fromList [("acc1", acc1 0 0), ("acc2", acc2 0 0)])
        )
        s2
    ...
    it "Can rename accounts" $ do
      ...
      assertEqual
        "Rename map"
        ( AccountMap
            (Map.fromList [("acc3", acc1 0 0), ("acc2", acc2 0 0)])
        )
        a
    ...
    it "Produces correct Commit/Nocommit" $ do
      let Right c1 = exec s2 $ getAccountStore (Just "acc1")
          ...
          Right c4 = execAcc (acc1 0 0) $ void (genExtAddress ctx)
    ...
    it "Updates indices when generating addresses" $ do
      let Right (Commit a1) =
            execAcc (acc1 0 0) $
              replicateM_ 10 (genExtAddress ctx)
                >> replicateM_ 7 (genIntAddress ctx)
